...
/// MMRealSRGAN 自然修復（強）模式
/// 適合：自然照片修復，效果較強
class MMRealSRGANUpscaler: ImageUpscaler {
    ...
    private static let modelDisplayName = L10n.string("mode.naturalStrong.name")
    private static let modelFileIdentifier = "MMRealSRGAN.mlmodel"
    ...
    enum UpscalerError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .modelNotFound:
                return L10n.formatted("ml.error.model_not_found", MMRealSRGANUpscaler.modelDisplayName, MMRealSRGANUpscaler.modelFileIdentifier)
            case .conversionFailed:
    ...
    init() throws {
        ...
        if let compiledURL = Bundle.main.url(forResource: "MMRealSRGAN", withExtension: "mlmodelc") {
            print("✅ 找到已編譯模型: MMRealSRGAN.mlmodelc")
            model = try MLModel(contentsOf: compiledURL, configuration: config)
            print("✅ 自然修復（強）模型載入成功")
        }
        // Fallback to .mlmodel
        else if let modelURL = Bundle.main.url(forResource: "MMRealSRGAN", withExtension: "mlmodel") {
            print("✅ 找到模型檔案: MMRealSRGAN.mlmodel")
            let compiledURL = try MLModel.compileModel(at: modelURL)
            model = try MLModel(contentsOf: compiledURL, configuration: config)
            print("✅ 自然修復（強）模型編譯並載入成功")
        } else {
            print("❌ 自然修復（強）模型未找到")
