...
/// AESRGAN 實驗模式
/// 適合：進階使用者，畫面較銳利，可能產生 artifacts
class AESRGANUpscaler: ImageUpscaler {
    ...
    private static let modelDisplayName = L10n.string("mode.experimental.name")
    private static let modelFileIdentifier = "aesrgan.mlmodel"
    ...
    enum UpscalerError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .modelNotFound:
                return L10n.formatted("ml.error.model_not_found", AESRGANUpscaler.modelDisplayName, AESRGANUpscaler.modelFileIdentifier)
            case .conversionFailed:
    ...
    init() throws {
        ...
        if let compiledURL = Bundle.main.url(forResource: "aesrgan", withExtension: "mlmodelc") {
            print("✅ 找到已編譯模型: aesrgan.mlmodelc")
            model = try MLModel(contentsOf: compiledURL, configuration: config)
            print("✅ 實驗模式模型載入成功")
        }
        // Fallback to .mlmodel
        else if let modelURL = Bundle.main.url(forResource: "aesrgan", withExtension: "mlmodel") {
            print("✅ 找到模型檔案: aesrgan.mlmodel")
